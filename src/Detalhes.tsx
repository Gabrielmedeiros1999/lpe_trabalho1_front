  ...
  return (
    <>
      <section className="flex mt-6 mx-auto flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-5xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
        <img className="object-cover w-full rounded-t-lg h-70 md:h-100 md:w-70 md:rounded-none md:rounded-s-lg "
          src={produto?.foto} alt="Foto do Jogo" />
        <div className="flex flex-col justify-between p-4 leading-normal">
           <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
            {produto?.tipo === "Jogo"
             ? `${produto?.nome} || Plataforma: ${produto?.plataforma.nome}`
             : produto?.nome}
            </h5>
           <h5 className="mb-2 text-xl tracking-tight text-gray-700 dark:text-gray-400">
           <span className="font-bold text-white">Descrição:</span> {produto?.descricao} 
          </h5>
          ...
          {cliente.id ?
            <>
              <h3 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">
                🙂Você pode fazer uma Proposta para este Produto!</h3>
