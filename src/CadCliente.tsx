...
export default function CadCliente() {
    ...
    async function cadastraCliente(data: Inputs) {
        ...
        const response = await
            fetch(`${apiUrl}/clientes/cadastro`, {
                        ...
                        <form className="space-y-4 md:space-y-6" 
                          onSubmit={handleSubmit(cadastraCliente)}>
                            ...
                            <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                                Já possui uma conta? <Link to="/login" className="font-medium text-green-600 hover:underline dark:text-green-500">Faça Login</Link>
                            </p>
                        </form>
                    </div>
                </div>
